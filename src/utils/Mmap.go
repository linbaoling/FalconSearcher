...
func (this *Mmap) ReadUInt64(start uint64) uint64 {
	...
	//return uint64(this.MmapBytes[start]) | (uint64(this.MmapBytes[start+1])<<8) | (uint64(this.MmapBytes[start+2])<<16) | (uint64(this.MmapBytes[start+3])<<24) | (uint64(this.MmapBytes[start+4])<<32) | (uint64(this.MmapBytes[start+5])<<40)| (uint64(this.MmapBytes[start+6])<<48) |(uint64(this.MmapBytes[start+7])<<56)
	return binary.LittleEndian.Uint64(this.MmapBytes[start : start+8])
}

func (this *Mmap) ReadUInt64Arry(start, len uint64) []DocIdNode {

	// arry = []uint64(this.MmapBytes[start:start+len*8])
...
func (this *Mmap) AppendStringWith32Bytes(value string, lens int64) error {
	...
	//fmt.Printf("Value : %v Lens : %v pinter %v\n",value,lens,this.FilePointer)
